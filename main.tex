...
\begin{document}
  ...
  \begin{frame}{Works covered in this talk}
    \begin{enumerate}
      ...
      \item \emph{Nesting Particle Filters for Experimental Design in Dynamical Systems}. Sahel Iqbal, Adrien Corenflos, Simo S\"arkk\"a, Hany Abdulsamad. 2024.
    \end{enumerate}
  \end{frame}
  ...
  \section{Markovian Score Climbing for MLE}
    ...
    \begin{frame}{Score Climbing For Likelihood Maximization}
      We aim to maximize the log-marginal likelihood w.r.t. $\phi$
      \begin{equation}
          \ell(\phi) := \log p(y_{0:T} = 1 \given \phi).
      \end{equation}
        using a stochastic gradient ascent procedure:
        \begin{equation*}
            \phi_k = \phi_{k-1} + \gamma_k \, \widehat{\nabla_\phi \ell}(\phi_{k-1}),
        \end{equation*}
        We know that the criteria necessary for convergence are~\citep{robbins1951stochastic}
        \begin{itemize}
          \item The gradient estimate should be unbiased:
            \begin{equation}
              \mathbb{E}\left[\widehat{\nabla_\phi \ell}(\phi_{k-1})\right] = \nabla_\phi \ell(\phi_{k-1})
            \end{equation}
          \item The step sizes must satisfy
            \begin{equation}
...
\begin{frame}{Markovian Score Climbing For Likelihood Maximization}
        \begin{itemize}
            ...
            \item We can do better by drawing samples from a Markov chain Monte Carlo kernel $\mathcal{K}$ ergodic w.r.t. $p_\phi(z_{0:T} \given y_{0:T})$ so that
            \begin{equation*}
                Z^n \sim \mathcal{K}(\cdot \given \mathcal{K}(\cdot \given \ldots, \phi), \phi) := \mathcal{K}^n(\cdot \given z_{0:T}, \phi).
            \end{equation*}
            \item The $p_\phi(z_{0:T} \given y_{0:T})$-ergodic MCMC kernel $\mathcal{K}$ guarantees a consistent estimates, no matter the number of samples $N$.
        \end{itemize}
    \end{frame}
\begin{frame}{Markovian Score Climbing For Likelihood Maximization}
        \begin{algorithm}[H]
    ...
    \begin{frame}{\textcolor{blue}{Conditional} SMC with backward sampling}
        \begin{algorithm}[H]
            ...
            \LinesNumbered
            \DontPrintSemicolon
            \KwIn{\textcolor{blue}{Trajectory $z_{0:T}$}}
