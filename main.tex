...
\begin{frame}{Markovian Score Climbing For Likelihood Maximization}
  ...
  \section{Inside-Out SMC\textsuperscript{2}}
    \begin{frame}{Preliminaries - IBIS}
      \begin{columns}
        \begin{column}{0.5\textwidth}
          \begin{itemize}
            \only<1,2>{
            \item For a model
              ...
              \end{gather}
              %
              how do we estimate the running filtered posteriors $p(\theta \mid z_{0:t})$?
            }
            \only<2-5>{
            \item We can use the \emph{iterated batch importance sampling}~(IBIS) algorithm from~\citet{chopin2002sequential}.
            }
            \only<3>{
